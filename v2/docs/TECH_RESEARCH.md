...
Checkpoint 2 rechecked the selected releases against npm before installation. Vite 8.3.0 and `@vitejs/plugin-react` 6.1.1 share the same Node requirement, the React plugin accepts Vite 8, React DOM 19.3 requires React 19.3, and `@types/three` 0.186.0 matches Three.js 0.186.0. `package-lock.json` is now the authority for the dependency graph that actually passed strict TypeScript checking, production bundling, and browser tests. The ranges in `package.json` remain upgrade policy rather than claims that future releases are already verified.
